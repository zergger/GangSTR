...
std::ostream& operator<< (std::ostream &out, HipStutterModel& model){
  out << "\n"
...
double HipStutterModel::get_parameter(bool in_frame, char parameter) const {
  switch(parameter){
  case 'U':
  ...
  default:
    std::cerr << "Invalid such stutter model parameter requested" << std::endl;
    exit(1);
  }
}
...
*/
double HipStutterModel::log_stutter_pmf(int sample_bps, int read_bps) const {
  double log_pmf;
...
void HipStutterModel::write(std::ostream& output) const {
  output << in_geom_  << "\t" << in_down_  << "\t" << in_up_  << "\t" 
	 << out_geom_ << "\t" << out_down_ << "\t" << out_up_ << "\t" << motif_len_ << std::endl;
}

HipStutterModel* HipStutterModel::read(std::istream& input){
  ...
  if (!(ss >> inframe_geom >> inframe_down >> inframe_up >> outframe_geom >> outframe_down >> outframe_up >> motif_len)){
    std::cerr << "Improperly formatted stutter model file" << std::endl;
    exit(1);
  }
  if (motif_len < 1){
    std::cerr << "Improperly formatted stutter model file. One or more entries have a motif length < 1" << std::endl;
    exit(1);
  }
  if (motif_len > 9){
    std::cerr << "Improperly formatted stutter model file. One or more entries have a motif length > 9" << std::endl;
    exit(1);
  }
  return new HipStutterModel(inframe_geom, inframe_up, inframe_down, outframe_geom, outframe_up, outframe_down, motif_len);
}
