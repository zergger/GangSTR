#ifndef HIP_STUTTER_MODEL_H_
#define HIP_STUTTER_MODEL_H_
...
class HipStutterModel {
 ...
 public:
  HipStutterModel(double inframe_geom,  double inframe_up,  double inframe_down, 
  ...
  bool parameters_within_threshold(HipStutterModel& other, double max_diff){
  ...
  friend std::ostream& operator<< (std::ostream &out, HipStutterModel& model);

  HipStutterModel* copy() const { return new HipStutterModel(in_geom_, in_up_, in_down_, out_geom_, out_up_, out_down_, motif_len_); }
  ...
  static HipStutterModel* read(std::istream& input);
};
