...
void show_help() {
  ...
  help_msg << "\nUsage: GangSTR [OPTIONS] "
	   ...
	   << "\t" << "--stutter-mode <string>       " << "\t" << "Stutter source: auto|learn|external|off. Default: " << options.stutter_mode << "\n"
	   << "\t" << "--stutter-model-tsv <file>    " << "\t" << "External stutter model table with columns chrom,start,end,period,motif,in_geom,in_up,in_down,out_geom,out_up,out_down" << "\n"
	   << "\t" << "--stutter-gate-max-adjacent-diff <int>" << "\t" << "Maximum allowed ENCLOSE count difference between small allele and small+1 before applying learned stutter reranking. Default: " << options.stutter_gate_max_adjacent_diff << "\n"
	   << "\t" << "--stutter-gate-min-adjacent-count <int>" << "\t" << "Minimum ENCLOSE count required for both small allele and small+1 before applying learned stutter reranking. Default: " << options.stutter_gate_min_adjacent_count << "\n"
...
void parse_commandline_options(int argc, char* argv[], Options* options) {
  enum LONG_OPTIONS {
    ...
    OPT_STUTMODE,
    OPT_STUTMODEL,
    OPT_STUTGATE_MAXDIFF,
    OPT_STUTGATE_MINCOUNT,
    OPT_NBSTRAP,
    OPT_BOOTSTRAP_ALPHA,
    OPT_RDPROB,
    OPT_OUTBS,
  ...
  static struct option long_options[] = {
    ...
    {"stutter-mode", required_argument, NULL, OPT_STUTMODE},
    {"stutter-model-tsv", required_argument, NULL, OPT_STUTMODEL},
    {"stutter-gate-max-adjacent-diff", required_argument, NULL, OPT_STUTGATE_MAXDIFF},
    {"stutter-gate-min-adjacent-count", required_argument, NULL, OPT_STUTGATE_MINCOUNT},
  ...
  while (ch != -1) {
    ...
    case OPT_STUTPR:
      ...
      break;
    case OPT_STUTMODE:
      options->stutter_mode = lowercase(optarg);
      break;
    case OPT_STUTMODEL:
      options->stutter_model_tsv = optarg;
      break;
    case OPT_STUTGATE_MAXDIFF:
      options->stutter_gate_max_adjacent_diff = atoi(optarg);
      break;
    case OPT_STUTGATE_MINCOUNT:
      options->stutter_gate_min_adjacent_count = atoi(optarg);
      break;
    case OPT_NBSTRAP:
  ...
  if (options->stutter_mode != "auto" &&
      options->stutter_mode != "learn" &&
      options->stutter_mode != "external" &&
      options->stutter_mode != "off") {
    PrintMessageDieOnError("--stutter-mode must be one of: auto, learn, external, off", M_ERROR, false);
  }
  if (options->stutter_mode == "external" && options->stutter_model_tsv.empty()) {
    PrintMessageDieOnError("--stutter-mode external requires --stutter-model-tsv", M_ERROR, false);
  }
  if (options->stutter_gate_max_adjacent_diff < 0) {
    PrintMessageDieOnError("--stutter-gate-max-adjacent-diff must be >= 0", M_ERROR, false);
  }
  if (options->stutter_gate_min_adjacent_count < 1) {
    PrintMessageDieOnError("--stutter-gate-min-adjacent-count must be >= 1", M_ERROR, false);
  }
...
int main(int argc, char* argv[]) {
  ...
  // Second pass: initialize stutter models
  if ((options.stutter_mode == "auto" || options.stutter_mode == "external") &&
      !options.stutter_model_tsv.empty()) {
    if (!genotyper.LoadExternalStutterModels(options.stutter_model_tsv)) {
      PrintMessageDieOnError("Error loading external stutter models", M_ERROR, false);
    }
  }

  if (options.stutter_mode == "auto" || options.stutter_mode == "learn") {
